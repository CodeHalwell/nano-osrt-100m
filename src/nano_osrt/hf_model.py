        ...
        **kwargs,
        ...
        for _ in range(max_new_tokens):
            ...
            if repetition_penalty != 1.0:
                # ⚡ Bolt: Vectorized element-wise operations instead of slow python
                # loops + tolist() to keep operations in optimized C++ backend and
                # prevent synchronization overhead.
                for b in range(generated.shape[0]):
                    unique_tokens = generated[b].unique()
                    valid_tokens = unique_tokens[unique_tokens < next_logits.shape[-1]]
                    if valid_tokens.numel() > 0:
                        token_logits = next_logits[b, valid_tokens]
