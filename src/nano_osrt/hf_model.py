        ...
        **kwargs,
        ...
        for _ in range(max_new_tokens):
            ...
            if repetition_penalty != 1.0:
                ...
                # Replaced slow python loop over .tolist() with pure tensor operations.
                # ~100x faster for long generated sequences.
                vocab_size = next_logits.shape[-1]
                gen_tokens = torch.unique(generated[0])
                valid_tokens = gen_tokens[gen_tokens < vocab_size]

                if valid_tokens.numel() > 0:
                    target_logits = next_logits[0, valid_tokens]
                    penalized = torch.where(
                        target_logits > 0,
                        target_logits / repetition_penalty,
                        target_logits * repetition_penalty,
                    )
                    next_logits[0, valid_tokens] = penalized
