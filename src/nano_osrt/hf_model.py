        ...
        **kwargs,
        ...
        for _ in range(max_new_tokens):
            ...
            if repetition_penalty != 1.0:
                ...
                # Avoid out-of-bounds scatter by sizing mask to max token ID
                max_id = max(vocab_size, int(generated.max().item() + 1))
                mask = torch.zeros(
                ...
                next_logits[mask] = torch.where(
                    next_logits[mask] > 0,
                    next_logits[mask] / repetition_penalty,
                    next_logits[mask] * repetition_penalty,
                )
