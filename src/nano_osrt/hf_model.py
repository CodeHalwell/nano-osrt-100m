        ...
        **kwargs,
        ...
        for _ in range(max_new_tokens):
            ...
            if repetition_penalty != 1.0:
                batch_size, vocab_size = next_logits.shape
                max_gen_id = max(vocab_size, int(generated.max().item()) + 1)

                # Create mask handling potential out-of-bounds tokens
                mask = torch.zeros(
                    batch_size, max_gen_id, dtype=torch.bool, device=next_logits.device
                )
