    ...
    def generate(
        ...
        **kwargs,
        ...
        for _ in range(max_new_tokens):
            ...
            # ⚡ Bolt: Vectorized repetition penalty to prevent O(N) loops
            # and fix batch size > 1 crashes
            if repetition_penalty != 1.0:
                vocab_size = next_logits.shape[-1]
                max_id = max(vocab_size, generated.max().item() + 1)
                mask = torch.zeros(
                    (generated.shape[0], max_id),
                    dtype=torch.bool,
                    device=next_logits.device,
                )
                mask.scatter_(1, generated, True)
            ...
            # ⚡ Bolt: Vectorized eos check to prevent batch size > 1 crashes
