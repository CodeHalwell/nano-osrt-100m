...
def apply_rope(x: Tensor, cos: Tensor, sin: Tensor) -> Tensor:
    ...
    d = x.shape[-1] // 2
    x1, x2 = x[..., :d], x[..., d:]
    # Calculate and concatenate results directly instead of allocating
    # intermediate full-size tensors to reduce memory bandwidth usage.
    cos_d, sin_d = cos[..., :d], sin[..., :d]
    return torch.cat([x1 * cos_d - x2 * sin_d, x2 * cos_d + x1 * sin_d], dim=-1)
