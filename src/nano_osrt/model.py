...
def apply_rope(x: Tensor, cos: Tensor, sin: Tensor) -> Tensor:
    ...
    d = x.shape[-1] // 2
    x1, x2 = x[..., :d], x[..., d:]
    # ⚡ Bolt: Optimize RoPE application by avoiding full-size intermediate
    # tensor allocations. Instead of creating x_rot and doing math on the full
    # concatenated tensor, we compute the two halves separately and concatenate
    # at the end. This reduces memory bandwidth usage and speeds up computation.
    out1 = x1 * cos[..., :d] - x2 * sin[..., :d]
    out2 = x2 * cos[..., d:] + x1 * sin[..., d:]
