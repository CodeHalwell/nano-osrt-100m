...
def apply_rope(x: Tensor, cos: Tensor, sin: Tensor) -> Tensor:
    ...
    # OPTIMIZATION: Calculate and concatenate the results directly
    # instead of allocating an intermediate full-size tensor `x_rot`.
    # This reduces memory bandwidth usage and allocations, which typically
    # improves execution latency on hardware like GPUs.
    return torch.cat(
        [x1 * cos[..., :d] - x2 * sin[..., :d], x2 * cos[..., d:] + x1 * sin[..., d:]],
        dim=-1,
    )
